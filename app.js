...
function drawVesselForPDF() {
    ...
    if (n_stages === 1) {
        ...
        stages_y.push(y_bottom_impeller);
        const top_impeller_limit = y_liquid + (d_px * 0.6);
        const available_span = y_bottom_impeller - top_impeller_limit;
        const stage_gap = Math.min(d_px * 1.2, available_span / (n_stages - 1));
